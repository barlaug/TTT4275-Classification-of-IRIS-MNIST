...
def extract_sets(data, targets, train_set_size, num_classes):
    ...
    t_train = np.zeros((num_classes, num_classes*train_set_size)) #3x90

    t_vec = np.zeros((num_classes, int(len(data)))) 
    print(targets.shape)
    ...
    print(t_vec)
    print(t_train)
    return train_samples, test_samples, t_train
...
def train_lin_model(train_samples, train_targets, alpha, num_iterations):
    ...
    print(W.shape)
    t_k = np.zeros((num_classes,1)) #np.array([[1 ,0 ,0] ,[0 ,1 ,0] ,[0 ,0 ,1]])
    ...
    for i in range(num_iterations):
        ...
        for k in range(int(len(train_samples))):
            ...
            grad2 = np.multiply(grad_g_MSE,grad_z_gk)
            print(grad.shape)
            print(grad2.shape)
...
if __name__ == '__main__':
    ...
    iterations = 5000
    train_set_size = 30
    train_set, test_set, t_train = extract_sets(iris_data, iris_targets, train_set_size, num_classes)
    for alpha in alphas:
